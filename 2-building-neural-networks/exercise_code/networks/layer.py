...
def affine_forward(x, w, b):
    ...
    ########################################################################
    inp_shape = x[0].shape
    new_shape = np.prod(inp_shape)
    num_batches = len(x)
    reshaped_input = np.empty((num_batches, new_shape))
    for i, batch in enumerate(x):
        reshape_batch = np.reshape(batch, (new_shape))
        reshaped_input[i] = reshape_batch
        
    for i, batch in enumerate(reshaped_input):
        affine_layer =  np.matmul(w.T, batch) + b
        out[i] = affine_layer
...
def affine_backward(dout, cache):
    ...
    #dw = np.average(np.matmul(dout, (w+b).T))
    reshaped_x = x.reshape((N,D)).T
    dw = np.matmul(reshaped_x, dout) / N
