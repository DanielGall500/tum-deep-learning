...
class LeakyRelu:
    ...
    def forward(self, x):
        ...
        ########################################################################
        relu_outputs, relu_cache = Relu().forward(x)
        outputs = np.where(relu_outputs > 0, relu_outputs, x*self.slope)
        cache = x
        pass
    ...
    def backward(self, dout, cache):
        ...
        ########################################################################
        dx = np.where(cache > 0, 1, self.slope)*dout
        pass
...
class Tanh:
    ...
    def backward(self, dout, cache):
        ...
        sigmoid = lambda x: 1 / (1 + np.exp(-x))
        tanh = lambda x: 2 * sigmoid(2*x) - 1
        dx = (1 - np.power(tanh(cache), 2))*dout
        pass
